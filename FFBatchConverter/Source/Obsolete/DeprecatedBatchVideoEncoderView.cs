...
public partial class DeprecatedBatchVideoEncoderView : View
{
	...
	private List<DeprecatedVideoEncoder> Encoders { get; } = [];
	...
	public DeprecatedBatchVideoEncoderView()
	{
		InitializeComponent();
	...
	private void OnAddFilesButtonOnClicked(object? sender, CancelEventArgs cancelEventArgs)
	{
		...
		foreach (string path in paths2)
		{
			...
			DeprecatedVideoEncoder e = new DeprecatedVideoEncoder(path, row);
	...
	private void OnEncoderInfoUpdate(DeprecatedVideoEncoder encoder, DataReceivedEventArgs? e)
	{
		// Update gui display
		Application.Invoke(() =>
		{
		...
		lock (EncodersLock)
		{
			...
			{
				DeprecatedVideoEncoder? next = Encoders.FirstOrDefault(e2 => e2.State == EncodingState.Pending);
